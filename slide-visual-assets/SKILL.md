...
description: >-
  Plan, generate, and route visual assets for single-file
  HTML slide decks, including host-native generated images, GPT Image 2
  ...
  slide generation whenever the environment supports image2, GPT Image 2,
  or host-native image generation and the user has not explicitly opted out.
...
bitmap assets when available. In host environments with native image
generation, call the host image tool directly. In local API environments, call
GPT Image 2 through the retained scripts. It does not own deck layout. It hands
generated image paths, aspect ratios, fallback decisions, and placement notes to the HTML
slide generator.

This is a **high-priority pre-generation asset production step**. Run it after
the deck plan, style/case direction, and typography are known, but before
writing the final HTML, whenever image2 / GPT Image 2 / host-native image
generation is available and the user has not explicitly said not to use
generated images. Its job is to supply concrete image files for slides that
benefit from them.
...
- host-native image generation exposed by the current environment, or
- runtime environment variables injected by host environment.
...
This module keeps the fuller `gpt-image-2` prompt library and scripts, but the
Package rules in this file are the adapter. Use upstream templates only after the
...
Do not request images for every slide. But for a normal polished deck, plan
at least the cover/chapter/product/evidence/diagram visuals that would materially
improve comprehension, memorability, or perceived polish.

## Mode Detection

Use the strongest available mode. If image2, GPT Image 2, or host-native image
generation is available, attempt real image generation by default for meaningful
deck visuals unless the user explicitly opts out. Do not wait for the user to
ask for images.

1. **Mode A · Host-native image generation**: if the host has a native image
   generation tool or image API, call it directly with the final prompt. In
   Codex, use the native image generation capability exposed by the host.
2. **Mode B · Local GPT Image 2 generation**: if a local image generation
   API/tool is configured, call GPT Image 2 using the retained scripts and save
   images to disk.
...
Do not pretend an image was generated if it was only prompted. Do not expose
image prompts to the user as a substitute for generated assets.

Full upstream mode and script guidance is retained at:

- `references/gpt-image-2/original-SKILL.md`
- `scripts/gpt-image-2/scripts/check-mode.js`
- `scripts/gpt-image-2/scripts/generate.js`
- `scripts/gpt-image-2/scripts/edit.js`

Use those scripts when the environment is configured for local GPT Image 2
...
## GPT Image 2 Generation Workflow

When actual image generation is needed:

1. Read this file and pick the asset role.
2. Pick one relevant generation template from
   `references/gpt-image-2/references/`.
...
4. Check whether the host environment exposes a native image generation
...
5. If no host-native image tool is available, run the local GPT Image 2 mode
   detector:

```bash
node slide-visual-assets/scripts/gpt-image-2/scripts/check-mode.js --json
```

6. If local GPT Image 2 generation is available, call:

```bash
node slide-visual-assets/scripts/gpt-image-2/scripts/generate.js \
...
9. For final single-file HTML delivery, either reference the local image path
   when Codex can package it, or embed the image as base64 when the deck must be
...
- Local GPT Image 2 scripts: 2-3 concurrent processes by default.
...
- the environment supports image2 / GPT Image 2 / host-native image generation,
...
- `references/gpt-image-2/references/poster-and-campaigns/editorial-cover.md`
- `references/gpt-image-2/references/poster-and-campaigns/banner-hero.md`
- `references/gpt-image-2/references/poster-and-campaigns/campaign-kv.md`
...
- `references/gpt-image-2/references/ui-mockups/landing-page-case-study.md`
- `references/gpt-image-2/references/ui-mockups/chat-interface-scene.md`
- `references/gpt-image-2/references/ui-mockups/product-card-overlay.md`
- `references/gpt-image-2/references/product-visuals/premium-studio-product.md`
- `references/gpt-image-2/references/product-visuals/lifestyle-product-scene.md`
...
- `references/gpt-image-2/references/technical-diagrams/system-architecture.md`
- `references/gpt-image-2/references/technical-diagrams/flowchart-decision.md`
- `references/gpt-image-2/references/technical-diagrams/sequence-diagram.md`
- `references/gpt-image-2/references/technical-diagrams/network-topology.md`
- `references/gpt-image-2/references/academic-figures/method-pipeline-overview.md`
...
- `references/gpt-image-2/references/infographics/bento-grid-infographic.md`
- `references/gpt-image-2/references/infographics/comparison-infographic.md`
- `references/gpt-image-2/references/infographics/kpi-dashboard-infographic.md`
- `references/gpt-image-2/references/slides-and-visual-docs/visual-report-page.md`
- `references/gpt-image-2/references/slides-and-visual-docs/educational-diagram-slide.md`
...
- Use generated host-native or GPT Image 2 outputs as local files or base64-embedded assets
...
- When the deck is generated inside Codex, prefer passing generated image paths
...
- If image generation was requested, did the skill actually attempt host-native
  image generation first, then local GPT Image 2 if needed, before falling back?
...
| Need | Read |
| --- | --- |
| Internal image request writing | `references/gpt-image-2/references/prompt-writing.md` |
| Slide/document visuals | `references/gpt-image-2/references/slides-and-visual-docs/` specific template |
| Editorial cover/KV | `references/gpt-image-2/references/poster-and-campaigns/` specific template |
| Product/UI mock | `references/gpt-image-2/references/ui-mockups/` or `product-visuals/` specific template |
| Technical architecture/flow | `references/gpt-image-2/references/technical-diagrams/` specific template |
| Academic figure | `references/gpt-image-2/references/academic-figures/` specific template |
| Infographic | `references/gpt-image-2/references/infographics/` specific template |
| Image editing | `references/gpt-image-2/references/editing-workflows/` specific template |
