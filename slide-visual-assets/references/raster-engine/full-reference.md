---
name: raster-engine
description: 面向 Raster Engine 的图像生成 / 编辑技能。可在 3 种环境下使用：(A) Garden 本地模式，通过 OpenAI 兼容接口直接出图并落盘；(B) Host-Native 模式，把本 Skill 当作提示词工程指引，把渲染好的 prompt 交给宿主 Agent 自带的图像工具出图；(C) Advisor 模式，宿主无任何图像工具时退化为高质量 prompt 顾问。涵盖 18 大类、80+ 个结构化模板，覆盖海报 / UI / 产品 / 信息图 / 学术图 / 技术架构图 / 漫画 / 头像 / 流程板 / 电影分镜 / IP 周边 / 编辑工作流等场景。
---

# Raster Engine

这是一个面向 Raster Engine 的聚焦型技能，在 3 种运行环境下都能用，但行为差异显著。**第一步必须先确定当前运行模式**。
...
```bash
node skills/raster-engine/scripts/check-mode.js
# 想拿结构化结果给上层程序用：
node skills/raster-engine/scripts/check-mode.js --json
```
...
- prompt 默认落盘到 `hanbao-raster-engine/prompt/`、图片落盘到 `hanbao-raster-engine/image/`。
- 这是最强的模式：你是图像工具的"持有者"。
...
- 你的工具集里出现 `image_generation` / `native image tool` / `dalle` / `nano_banana` / `mcp__*image*` / `make_image` / 类似名字
...
4. 如用户希望可顺手把 prompt 文件保存到 `hanbao-raster-engine/prompt/`，但图片去向由宿主决定，不强制。
...
2. 把最终 prompt **直接打印给用户** + 保存一份到 `hanbao-raster-engine/prompt/<task-slug>-<timestamp>.md`。
3. 附一句简短的"如何使用"建议（如：丢进 ChatGPT / Midjourney / DALL·E / Sora / Nano Banana / 自己后端 / 第三方 Raster Engine 网关）。
...
- `OPENAI_IMAGE_MODEL` — 默认 `raster-engine`，可换成网关支持的型号（如 `gpt-image-1` / `dall-e-3`）。
...
- 提示词目录：`hanbao-raster-engine/prompt/`（**A / B / C 三种模式都建议用**，方便复用与版本管理）
- 图片目录：`hanbao-raster-engine/image/`（**仅 Mode A 使用**；Mode B 由宿主决定，Mode C 不产生图）
...
- 提示词：`hanbao-raster-engine/prompt/<task-slug>-<timestamp>.md`
- 图片：`hanbao-raster-engine/image/<task-slug>-<timestamp>.png`
...
- `hanbao-raster-engine/prompt/live-commerce-ui-20260424-153045.md`
- `hanbao-raster-engine/image/live-commerce-ui-20260424-153045.png`
- `hanbao-raster-engine/prompt/vr-headset-exploded-view-20260424-153102.md`
- `hanbao-raster-engine/image/vr-headset-exploded-view-20260424-153102.png`
...
2. 如果用户直接给的是文本 prompt，也要先把最终 prompt 保存到 `hanbao-raster-engine/prompt/`。
...
2. 否则默认保存到 `hanbao-raster-engine/image/`。
...
```bash
node skills/raster-engine/scripts/generate.js \
...
```bash
node skills/raster-engine/scripts/generate.js \
  --promptfile hanbao-raster-engine/prompt/poster-20260424-153045.md
```

### 3. 编辑已有图片（Mode A）

```bash
node skills/raster-engine/scripts/edit.js \
...
- **Mode C**：渲染好最终 prompt → 保存到 `hanbao-raster-engine/prompt/<task-slug>-<timestamp>.md` → 把内容直接展示给用户 → 提示用户在哪些图像工具中可以直接复用。
...
7-A. **Mode A**：把最终 prompt 保存到 `hanbao-raster-engine/prompt/`，调用 `scripts/generate.js` 或 `scripts/edit.js`，图片落到 `hanbao-raster-engine/image/`。
7-B. **Mode B**：把最终 prompt 直接传给宿主的图像工具调用；按需保存 prompt 副本到 `hanbao-raster-engine/prompt/`。
7-C. **Mode C**：把最终 prompt 保存到 `hanbao-raster-engine/prompt/<task-slug>-<timestamp>.md`，并把完整 prompt 在对话中展示给用户，附一句简短的"如何使用 / 推荐工具"建议。
