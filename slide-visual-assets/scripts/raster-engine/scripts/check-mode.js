...
if (gardenEnabled && apiKey) {
  ...
  summary =
    "ENABLE_GARDEN_IMAGEGEN 已开，但运行时环境没有注入 OPENAI_API_KEY。不要读取本地配置文件；改用宿主图像能力或降级到 MODE C。";
} else {
  mode = "B-or-C";
  recommendation = "host-or-advisor";
  summary =
    "MODE B / C · 未启用本地 API。若宿主 Agent 自带图像工具（image_generation / dalle / mcp__*image* 等）→ MODE B：把 prompt 交给宿主出图。若宿主无图像工具 → MODE C：仅产出高质量 prompt 给用户。";
}
...
if (wantJson) {
  ...
  console.log("--- raster-engine runtime mode ---");
