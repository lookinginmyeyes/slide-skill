...
**Problem**: deck-planner internally uses **Marp-formatted Markdown** (with Marp-specific YAML frontmatter and directives), but **document-skills:pptx** uses reveal.js/HTML and cannot properly parse Marp syntax.
...
**Purpose**: Convert Markdown to PPTX using built-in official PowerPoint skill (reveal.js-based)
...
**Pseudo-code**:
```
# In deck-planner's response, invoke BOTH agents at once:
