...
> 基于 高品质参考三支产品动画（design workflow demo / agent runtime Desktop / document workflow demo）
> 的深度拆解，提炼出的"高品质"动画设计规则。
>
...
**你是一个研究过 Reference Lab / Apple / Pentagram / Field.io 运动档案的 motion designer。**
...
| "这不像是 AI 做的" | great+ | 你触到了 Reference Lab 的门槛 |
...
真实世界的物体有质量、有惯性、有弹性、会溢出。Reference Lab 三支片子的「高级感」根源，
...
Reference Lab 三支片子的所有动效都用带「阻尼感」的贝塞尔曲线。默认的 cubic easeOut
...
Reference Lab 三支片子没有一支用 `#FFFFFF` 或 `#000000` 做主底色。**带色温的中性色**
...
- design workflow demo 展示 tweak 参数、拖滑块（不是一键生成完美结果）
- agent runtime 展示代码报错 + AI 修复（不是一次成功）
- document workflow demo 展示 Redline 红删绿增的修改过程（不是直接给最终稿）
...
Reference Lab 三支片子的 Logo 出场**都不是简单 fade-in**，是**前一个视觉元素形变而来**。
...
Disney 12 原则中的 3 条。Reference Lab 用得很显式：
...
### 配方 A · Apple Keynote 戏剧式（design workflow demo 类）
...
### 配方 C · 办公效率叙事式（document workflow demo 类）
...
- 高品质参考动画拆解：花叔项目目录的 `参考动画/BEST-PRACTICES.md`
- Reference Lab 音频拆解：同目录 `AUDIO-BEST-PRACTICES.md`
