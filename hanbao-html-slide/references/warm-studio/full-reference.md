...
**触发条件**：任务涉及具体品牌——用户提了产品名/公司名/明确客户（Stripe、Linear、Reference Lab、Notion、BrandX、DJI、自家公司等），不论用户是否主动提供了品牌资料。
...
**品牌多切面**：同一品牌的官网营销色和产品 UI 色经常不同（BrandX 官网暖米+橙，产品 UI 是 Charcoal + Lime）。**两套都是真的**——根据交付场景选合适的切面。
...
### 禁区
- <明确不能做的：比如 BrandX 不用蓝色、Stripe 不用低饱和暖色>
...
- **BrandX 设计**：把产品截图里演示品牌的喜茶红当成 BrandX 自己的色——差点毁整个设计
...
- ✅ 配图优先 AI 生成（Gemini / Flash / BrandX），HTML 截图仅在精确数据表格时用：AI 生成的图比 SVG 手画准确，比 HTML 截图有质感
...
**真实素材优先原则**（涉及用户本人/产品时）：
1. 先查用户配置的**私有 memory 路径**下的 `personal-asset-index.json`（agent runtime 默认在 `~/.claude/memory/`；其他 agent 按其自身约定）
...
| **动画的正向设计语法**（高品质叙事/运动/节奏/表达风格）| `references/animation-best-practices.md`（5 段叙事+Expo easing+运动语言 8 条+3 种场景配方）|
...
| **Gallery Ripple + Multi-Focus 场景哲学**（当素材 20+ 同质+场景需表达「规模×深度」时优先用；含前置条件、技术配方、5 个可复用模式）| `references/hero-animation-case-study.md`（warm-studio hero v9 蒸馏）|

## 跨 Agent 环境适配说明

本 skill 设计为 **agent-agnostic**——agent runtime、Codex、Cursor、Trae、OpenClaw、Hermes Agent 或任何支持 markdown-based skill 的 agent 都可以使用。以下是和原生「设计型 IDE」（如 browser artifact environment Artifacts）对比时的通用差异处理方式：
