...
2. Direction gate: ask the three-path entry question unless the user explicitly requested direct generation. If the user chooses style or template/case selection, collect that concrete choice before continuing.
...
- If the user chooses template/case selection, offer 2-3 concise template/case direction options next. Phrase each option by deck scenario, structure, and what the viewer will understand faster.
...
- Ask the user for typography only after a specific style option or a specific template/case option is chosen. `Editorial Luxe` is the premium serif option, but it is not automatic.
